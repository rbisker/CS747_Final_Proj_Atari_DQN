...
class Agent():
    ...
    def update_target_net(self):
        ...
        self.target_net.load_state_dict(self.policy_net.state_dict())
        # self.target_net.load_state_dict(self.policy_net._orig_mod.state_dict())  #use with torch.compile()
    ...
    def load_checkpoint(self, run_name, episode, dir="./checkpoints", compiled_policy=True, compiled_target=False):
        path = os.path.join(dir, f"{run_name}_{episode}_checkpoint.pt")
        checkpoint = torch.load(path)

        # === Re-instantiate base models ===
        self.policy_net = DQN(self.action_size).to(device)
        self.target_net = DQN(self.action_size).to(device)

        if compiled_policy:
            # Load into uncompiled model first
            self.policy_net.load_state_dict({k.replace("_orig_mod.", ""): v for k, v in checkpoint['policy_net'].items()})
            # # THEN compile
        ...
        if compiled_target:
            self.target_net.load_state_dict({k.replace("_orig_mod.", ""): v for k, v in checkpoint['target_net'].items()})
        ...
        self.epsilon = checkpoint['metadata']['epsilon']
