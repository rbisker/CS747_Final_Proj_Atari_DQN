...
class DQN_DualBranch(nn.Module):
    ...
    #     x_full = self.bn1_full(self.conv1_full(full_frames))
    #     x_diff = self.bn1_diff(self.conv1_diff(diff_frames))

    #     print(f"[DEBUG] Full branch: mean={x_full.mean().item():.4f}, std={x_full.std().item():.4f}")
    #     print(f"[DEBUG] Diff branch: mean={x_diff.mean().item():.4f}, std={x_diff.std().item():.4f}")
